...
import { ArrowUpRight, Check, Clipboard, Database, FileText, Landmark, MapPin, Search, TrendingUp, WalletCards } from 'lucide-react';
...
export default function Home() {
  ...
  return <main>
    ...
    <div id="top" className="shell page">
      ...
      <section className="details-grid"><article className="detail-card"><div className="icon"><WalletCards /></div><div><p className="eyebrow">계약 후 비용</p><h2>매월 확인할 항목</h2></div><ul>{report.fees.monthly.map((fee) => <li key={fee.label}><span>{fee.label}</span><b>{fee.value}</b></li>)}</ul><p className="card-note">정보공개서 표기 기준. 앱 수수료·교육 등 별도 항목도 확인하세요.</p></article><article className="detail-card"><div className="icon"><Landmark /></div><div><p className="eyebrow">가맹본부 재무</p><h2>{report.brand.company} 재무현황</h2></div><ul><li><span>매출</span><b>{eok(report.headquarters.revenueThousandWon)}</b></li><li><span>영업이익률</span><b>{calculated.operatingMargin.toFixed(1)}%</b></li><li><span>부채 / 자본</span><b>{calculated.debtToEquity.toFixed(1)}%</b></li></ul><p className="card-note">개별 가맹점의 이익·수익성을 뜻하지 않습니다.</p></article><article className="detail-card contract-card"><div className="icon"><FileText /></div><div><p className="eyebrow">계약 조건</p><h2>정보공개서 원문에서 추출</h2></div><ul>{report.contract.items.map((item) => <li key={item.label}><div><span>{item.label}</span><b>{item.value}</b></div><q>{item.evidence}</q></li>)}</ul><p className="card-note">공정위 등록번호 {report.contract.registrationSn} 정보공개서의 해당 조항 문장을 규칙으로 추출한 값이며, 인용문이 원문 근거입니다. 계약서 최종본과 반드시 대조하세요.</p></article></section>
